...
for line in stats:
    ...
    mse_allmadis = line_split[7] #mean squared error of all MADIS obs
    variance_allmadis = line_split[8] #variance of stationary MADIS obs
    try:
        mse_gridobs = line_split[9] #mean squared error of gridded obs
        variance_gridobs = line_split[10] #variance of gridded obs
        mse_allgrid = line_split[11] #mean squared error of entire grid
        variance_allgrid = line_split[12] #variance of entire grid
    except:
        pass
    ...
    stats_dict[var][ens][efa]['MSE'].append(float(mse_stationary))
    stats_dict[var][ens][efa]['Ensemble_Variance'].append(float(variance_stationary))

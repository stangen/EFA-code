...
for o, o_type in enumerate(ob_type):
    efa = Run_efa(datestr,ensemble_type,variables,var_dict[o_type])
    if o == 0:
        statecls, lats, lons, elevs = efa.load_data()

#loop to add observations of both altimeter and 2-m temperature
...
outfile = outdir+year+'-'+month+'-'+day+'_'+hour+'_'+ensemble_type+'ALT_T2M.nc'
